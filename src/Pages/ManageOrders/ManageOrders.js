...
const ManageOrders = () => {
  ...
  const handleCancellation = (id) => {
    ...
    if (confirmDelete) {
      fetch(`https://wanderlust-server-side.vercel.app/booking/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
      ...
      <div className="container mx-auto shadow-2xl my-10 rounded-xl">
        ...
        <div className="container xl:max-w-max px-8 py-8 xl:py-16 mx-auto bg-white">
          {allOrders.map((order, index) => (
            <div
              className="flex bg-green-50 shadow-2xl rounded-lg my-4"
              key={order._id}
            >
              <div className="flex-1 grid lg:grid-cols-6 gap-6 grid-flow-row">
                <div>
                  <img
                    className="xl:h-32 xl:rounded-l-lg"
                    src={order.plan.imageURL}
                    alt=""
                  />
                </div>
                <h2 className="m-auto text-xl xl:col-span-2 font-semibold">
                  {order.plan.title}
                </h2>
                <div className="flex justify-around">
                  <h2 className="m-auto text-xl font-semibold text-red-500">
                    ${order.plan.price}
                  </h2>
                  <span
                    className={
                      order.status
                        ? "bg-green-500 m-auto px-4 py-2 rounded"
                        : "bg-yellow-600 m-auto px-4 py-2 rounded"
                    }
                  >
                ...
                <div className="flex justify-around pb-3 md:pb-0">
                  <button
                    ...
                    className={
                      order.status
                        ? "bg-green-500 opacity-25 px-4 py-2 rounded text-white m-auto"
                        : "bg-green-500 hover:bg-green-600 px-4 py-2 rounded text-white m-auto"
                    }
                  ...
                  <button
                    ...
                    className="bg-red-500 hover:bg-red-600 px-4 py-2 rounded text-white m-auto"
                  >
                    Delete
                  </button>
                </div>
